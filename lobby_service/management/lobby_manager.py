...
class LobbyManager:
    ...
    def __init__(self, db: Session, redis_cache: redis.RedisCluster):
        self.db = db
        self.redis_cache = redis_cache

    def create(self, lobby: LobbyBaseDto):
        existing_lobby = self.db.query(Lobby).filter(
            Lobby.train_id == lobby.train_id).first()

        if existing_lobby:
            raise HTTPException(
                status_code=400, detail="Lobby with this train_id already exists")
    ...
    def get_all(self):
        ...
        if cached_lobbies:
            ...
            lobby_dtos = [LobbyInfoDto.model_validate(
                lobby) for lobby in lobbies]
            self.redis_cache.setex(name="lobbies", time=120, value=json.dumps(
                [lobby.model_dump() for lobby in lobby_dtos]))
            return lobby_dtos
    ...
    def delete(self, lobby_id: int):
        ...
        if db_lobby is None:
            raise HTTPException(
                status_code=404, detail="Lobby to delete not found")
